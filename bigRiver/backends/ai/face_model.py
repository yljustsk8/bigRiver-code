...
def face_identify_urls(img_urls):
    if len(img_urls)!=3:
        return 0
    res=[]
    for img_url in img_urls:
        ...
        if img is None:
            return 0
        user=face_identify(img)
        print("user",user)
        if user is None:
            return 0
        res.append(user)
    if len(res)!=3 or res[0]!=res[1] or res[0]!=res[2] :
        return 0
    return res[0]
...
def is_useful(img_url):
    ...
    if face is None or face.shape!=(128,128):
        return False
    return True
def useful_imgs(img_urls,temp_save_path):
    imgs=[]
    for img_url in img_urls:
        img=cv2.imread(img_url)
        if img is None:
    ...
    if len(imgs)==0:
        return 0
    faces=imgs2faces(imgs)
    save_faces(temp_save_path,faces)
    if faces is None:
        return 0
    else:
